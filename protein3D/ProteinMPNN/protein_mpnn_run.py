...
def main(args):
    ...
    #checkpoint_path = os.path.join("vanilla_model_weights", "v_48_020.pt")
    checkpoint_path = os.path.join("ProteinMPNN/vanilla_model_weights", "v_48_020.pt")
...
#    argparser.add_argument("--fixed_positions_jsonl", type=str, default='', help="Path to a dictionary with fixed positions")
    ...
    argparser.add_argument("--pdb_path", type=str, default="ProteinMPNN/chain_C_only.pdb", help="Path to a single PDB to be designed")
    argparser.add_argument("--out_folder", type=str, default="output", help="Path to output folder")
    argparser.add_argument("--fixed_positions_jsonl", type=str, default="fixed_positions.jsonl", help="Path to fixed positions JSON file")
    argparser.add_argument("--chain_id_jsonl", type=str, default="chain_ids.jsonl", help="Path to chain ID JSON file")
    argparser.add_argument("--sampling_temp", type=str, default="0.2", help="Sampling temperature for amino acids")
    #argparser.add_argument("--tied_positions_jsonl", type=str, default='', help="Path to a dictionary with tied positions")

    import sys
    if len(sys.argv) == 1:  # No arguments provided, use defaults
        args = argparser.parse_args([])
    else:  # Arguments provided via CLI
        args = argparser.parse_args()
